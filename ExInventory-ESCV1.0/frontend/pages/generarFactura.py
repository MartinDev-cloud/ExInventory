...
class GenerarFactura(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.lbl_iva = ttk.Label(self.frame_resumen, text="IVA (19%): 0.00")
        self.lbl_iva.pack(anchor="w", padx=10, pady=2)
        ...
        self.btn_generar = tk.Button(self, text="Generar Factura", bg="#28a745", fg="white",
                                     font=("Segoe UI", 10, "bold"), activebackground="#1e7e34",
                                     activeforeground="white", relief="flat",
                                     padx=15, pady=8, cursor="hand2", command=self.generar_factura)
    ...
    def actualizar_precio_desde_db(self, prod):
        ...
        for s in self.servicios_db:
            if s.get("numeroHabitacion") == nombre:
                prod["valorUnitario"].set(s.get("precioHabitacion", 0))  # Asumiendo que servicios tienen (precioHabitacion) variable
                return
        # Buscar en platillos
        for pl in self.platillos_db:
            if pl.get("nombrePlato") == nombre:
                prod["valorUnitario"].set(pl.get("precioVenta", 0))  # Asumiendo que platillos tienen (precioVenta) variable
                return
    ...
    def eliminar_producto(self, index):
        if len(self.productos) > 1:
            ...
            messagebox.showwarning("Advertencia", "Debe haber al menos un producto en la factura.")
    ...
    def actualizar_resumen(self):
        ...
        self.lbl_iva.config(text=f"IVA (19%): {self.format_currency(iva)}")
    ...
    def cargar_datos(self, documento):
        ...
        self.tipo_documento = documento.get("tipo", "Factura")
    ...
    def generar_factura(self):
        ...
        email = self.cliente["email"].get().strip()
        ...
        try:
            if self.editar_id:
                url = f"http://127.0.0.1:5000/facturas/{self.editar_id}"
                r = requests.put(url, json=factura)
                ...
                url = "http://127.0.0.1:5000/facturas"
                r = requests.post(url, json=factura)
                if r.status_code == 200:
                    ...
                    self.productos.clear()
                    self.add_producto()
                    self.btn_generar.config(text="Generar Factura")
                else:
                    messagebox.showerror("Error", r.text)
